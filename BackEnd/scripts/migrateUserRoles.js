...
(async () => {
    try {
        if (!process.env.MONGO_URI) {
            console.error("[migrate:user-roles] MONGO_URI is not set — aborting.");
            process.exit(1);
        }
        ...
        // empty array also gets the baseline. Same idempotency story.
        const empty = await User.updateMany(
            { roles: { $size: 0 } },
            { $set: { roles: ["peer_learner"] }, $inc: { rolesVersion: 0 } }
        );
