...
exports.submitRating = async (req, res) => {
    try {
        ...
        // Only the FIRST time this reviewer rates this user should count toward
        // their review missions — otherwise editing an existing review would
        // re-fire mission progress and let someone farm the review missions
        // (incl. "review 3 DIFFERENT partners") by re-submitting one review.
        const alreadyRated = await Rating.exists({ fromUser: req.user.id, toUser: toUserId });
        ...
        // Fire-and-forget — never affects the response. Guarded so only a NEW
        // review credits mission progress (edits to an existing one don't).
        if (!alreadyRated) {
