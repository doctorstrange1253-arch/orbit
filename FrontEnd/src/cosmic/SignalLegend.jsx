 ...
 */
import { Orbit as OrbitIcon, Zap } from 'lucide-react';
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-slate-900/30 p-4">
      ...
      <div className="grid gap-2.5 sm:grid-cols-3">
        {SIGNALS.map(({ key, Icon, photon, color, label, what }) => (
          <div key={key} className="rounded-xl border border-white/10 bg-slate-950/40 p-3">
            <div className="flex items-center gap-1.5 mb-1">
              {photon ? <PhotonIcon size={15} animated={false} /> : <Icon size={15} style={ { color } } />}
