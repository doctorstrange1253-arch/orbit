...
const ChatWindow = ({ otherUser, onBack, onlineUsers, isExpanded }) => {
  ...
  const otherTypingClearRef = useRef(null); // safety timer so a stuck "typing…" can't hang forever
  ...
  useEffect(() => {
    ...
    const handleTypingStart = ({ senderId }) => {
      if (senderId !== otherUser._id) return;
      setOtherUserTyping(true);
      // Safety net: if the matching typing-stop is ever lost (peer disconnects,
      // app backgrounded, dropped packet), auto-clear so it can't hang forever.
      if (otherTypingClearRef.current) clearTimeout(otherTypingClearRef.current);
      otherTypingClearRef.current = setTimeout(() => setOtherUserTyping(false), 6000);
    };
  ...
  const sendMutation = useMutation({
    ...
    onSuccess: (data) => {
      // The socket path resolves to null — the server echoes 'new-message', which
      // reconciles the optimistic temp bubble in that handler. The HTTP fallback
      // (socket down) returns the saved message directly and gets NO socket echo
      // back to the sender, so reconcile the temp bubble here too, or it lingers
      // with a fake temp id until the next refetch.
      if (!data?._id) return;
      queryClient.setQueryData(['messages', otherUser._id], (oldData) => {
        if (oldData && oldData.messages) {
          if (oldData.messages.some(m => m._id === data._id)) return oldData;
          const filtered = oldData.messages.filter(m => !(m._id.startsWith('temp-') && m.content === data.content));
          return { ...oldData, messages: [...filtered, data] };
        }
        const old = Array.isArray(oldData) ? oldData : [];
        if (old.some(m => m._id === data._id)) return old;
        const filteredLegacy = old.filter(m => !(m._id.startsWith('temp-') && m.content === data.content));
        return [...filteredLegacy, data];
      });
      queryClient.invalidateQueries({ queryKey: ['conversations'] });
      queryClient.invalidateQueries({ queryKey: ['unread-count'] });
    },
