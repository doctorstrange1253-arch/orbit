...
const FlyTo = ({ center, zoom }) => {
  const map = useMap();
  // Fly only when the target actually changes — running this during render
  // re-centered the map on every re-render, yanking users back while panning.
  useEffect(() => {
...
const NearbyMap = () => {
  ...
  const autoLocate = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      // Without a timeout, getCurrentPosition can hang forever on mobile/APK
      // WebView — leaving the user stuck on “Detecting your location…”.
