...
const Settings = () => {
  ...
  const handleReset = () => {
    playClick();
    reset();
    // reset() restores the default cyan-purple (a DARK theme); force dark mode too
    // so themeStore.isDark + data-mode stay in sync with the theme. Otherwise a
    // user who resets from light mode is left mismatched (dark theme label on a
    // light-mode palette) and 'reset to defaults' fails to restore dark mode.
    setDarkMode(true);
