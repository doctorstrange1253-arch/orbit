...
export async function initPushNotifications(navigateFn) {
  ...
  try {
    ...
    // Android 8+: ensure the high-importance channel referenced by the FCM
    // default-channel manifest meta-data exists, so killed/background pushes
    // post as heads-up alerts with sound. Channels persist once created.
    try {
      await PushNotifications.createChannel({
        id: 'orbit_default',
        name: 'Orbit',
        description: 'Matches, messages, and mission alerts',
        importance: 4,
        visibility: 1,
        sound: 'default',
        vibration: true,
      ...
      /* older devices / web: no channel API — safe to skip */
    }
